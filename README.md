...
2. Run `Build.ps1` and enter the version in `X.X.X.X` format when prompted. The build script will produce `SingleFileFormation.pak`, `info.json` and `SingleFileFormation.zip` in the `Output` folder.
